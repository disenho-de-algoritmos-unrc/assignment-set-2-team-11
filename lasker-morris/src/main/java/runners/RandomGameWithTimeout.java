...
import engines.ThreadedRandomEngine;
...
public class RandomGameWithTimeout {
	...
	public static void main(String[] args) {
		...
		while (!error && !state.isFinal() && moves < MAXMOVES) {
		    ...
		    try {
                TimeLimitedCodeBlock.runWithTimeout(new ThreadedRandomEngine(state), TIMEOUT, TimeUnit.SECONDS);
            }
            catch (Exception e) {
                // Got timeout.
            }
            LaskerMorrisGameState result = ThreadedRandomEngine.result;
